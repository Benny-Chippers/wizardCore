module mem_memory #(
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset_n) begin
            ...
            if(en_IF) begin
                o_instr <= mem_array[i_instrAddr[16:2]]; // Word-aligned access for instruction fetch
            end
        end
    end
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset_n) begin
                ...
                unique case (i_ctrlMEM.size)
                    2'b00: begin
                        unique case (i_memAddr[1:0]) // Byte-aligned access
                            2'b00: mem_array[i_memAddr[16:2]][7:0] <= i_writeData[7:0];
                            2'b01: mem_array[i_memAddr[16:2]][15:8] <= i_writeData[7:0];
                            2'b10: mem_array[i_memAddr[16:2]][23:16] <= i_writeData[7:0];
                            2'b11: mem_array[i_memAddr[16:2]][31:24] <= i_writeData[7:0];
                        endcase
                    end
                    2'b01: begin
                        unique case (i_memAddr[1]) // Halfword-aligned access
                            1'b0: mem_array[i_memAddr[16:2]][15:0] <= i_writeData[15:0];
                            1'b1: mem_array[i_memAddr[16:2]][31:16] <= i_writeData[15:0];
                        endcase
                    end
                    2'b10: mem_array[i_memAddr[16:2]] <= i_writeData; // Word-aligned access
                    default: begin
                        //nothing
                    end
                endcase
            ...
            // Read
            w_tempRD <= mem_array[i_memAddr[16:2]];
        end
    end
